...
Constants.STAGES_PER_LEVEL_MIN = 5;
Constants.STAGES_PER_LEVEL_MAX = 7;
...
GameConfig.attributes.add('itemHighestYPositionThreshold', {
    type: 'number',
    description: 'object will be destroyed if its y-position will be above that value',
    default: 25,
    min: 0,
    max: 100
});
...
Item.prototype.update = function(dt) {
    ...
    if(this.entity.getPosition().y < GameConfig.getAttribute('itemLowestYPositionThreshold') || this.entity.getPosition().y > GameConfig.getAttribute('itemHighestYPositionThreshold') ) {
...
LevelBuilder.prototype.loadStage = function (levelIndex, stageIndex) {
    ...
    const applyRotation = this.entity.script.levelManager.applyLevelRotation && !stagePrefab.script.stageConfig.disableRandomRotation;
...
LevelManager.prototype.getStagePrefab = function (levelIndex, stageIndex) {
    if (this.loadTestStage) {
        ...
        if (!stage) {
            return Utils.getRandomItem(this.regularStages);
        }
        return stage;
    }
    return null;
...
StageConfig.attributes.add('disableRandomRotation', {
...
LevelProgressContainer.prototype.rebuildInterface = function() {   
    ...
    /* scale to fint width in portrait mode */
     if(this.orientation === LevelProgressContainer.PORTRAIT) {
         let targetBarScale = pc.math.clamp(ScaleManager.screenRatio / ScaleManager.SCREEN_RATIO_MIN, 0.5, 1);
         const barClampedScale = pc.math.clamp(500 / ((totalStages + 3) * this.STAGE_PLATE_WIDTH), 0.75, 1);
         if(barClampedScale < 1) targetBarScale = Math.min(targetBarScale, barClampedScale)
...
pc.AppBase.prototype.unpauseGame = function (forced) {
    if (isPageVisible && (!adIsShowing || forced)) {
        ...
        const restoredVolume = (this.soundVolumeBeforePaused === undefined) ? (SoundController.masterVolume || 1) : this.soundVolumeBeforePaused;
        this.fire('audio:setMasterVolume', restoredVolume);
...
ScaleManager.prototype.update = function (dt) {
    ...
    // this.app.root.findByName("DebugText").element.text = '' + ~~this.app.graphicsDevice.width + 'x' + ~~this.app.graphicsDevice.height + ' x' + this.app.graphicsDevice.maxPixelRatio +  ' (' + ~~this.app.graphicsDevice.clientRect.width + 'x' + ~~this.app.graphicsDevice.clientRect.height + ')';
};
...
ScaleManager.prototype.updateScreenSize = function () {
    ...
    this.app.graphicsDevice.resizeCanvas(Math.floor(width), Math.floor(height));
...
SettingsPanel.prototype.toggleSettings = function () {
    ...
    this.entity.buttonSettings.tween(this.entity.buttonSettings)
        ...
        .onUpdate( () => this.entity.buttonSettings.setLocalEulerAngles(0, 0, this.entity.buttonSettings.angle))
        .start();
...
MainMenu.prototype.initialize = function () {
    ...
    this.entity.headingIcon = this.entity.findByName("HeadingIcon");
    this.entity.headingIconRu = this.entity.findByName("HeadingIcon_ru");
    ...
    this.entity.show = function () {
        ...
        let headingIcon = (window.famobi.getCurrentLanguage() === 'ru') ? this.headingIconRu : this.headingIcon;
        if(window.famobi.getCurrentLanguage() === 'ru') {
            this.headingIcon.enabled = false;
            this.headingIconRu.enabled = true;
        } else {
            this.headingIcon.enabled = true;
            this.headingIconRu.enabled = false;
        }
        headingIcon.element.opacity = 0.0;
        var headingAppearingTween =
            headingIcon.tween(headingIcon.element)
    ...
    this.entity.hide = function () {
        ...
        /* tween heading icon */
        headingIcon.element.opacity = 1.0;
        var headingAppearingTween =
            headingIcon.tween(headingIcon.element)
                .to({ opacity: 0.0 }, 0.35, pc.SineOut)
                .start();

        var position = headingIcon.getLocalPosition();
        headingIcon.tween(headingIcon.getLocalPosition())
...
MainMenu.prototype.update = function (dt) {
    if (skipTitleScreen()) {
        this.entity.clickZone.enabled = this.entity.handContainer.enabled = this.entity.headingIcon.enabled = this.entity.headingIconRu.enabled = false;
    }
};
...
FamobiSafeArea.prototype.postInitialize = function () {
    this._onCanvasResize();
};
